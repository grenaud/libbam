...
int main (int argc, char *argv[]) {
    ...
    const string usage=string(string(argv[0])+" [options] input.bam out.bam"+"\n\n"+
			      "\tIlluminas software produces unresolved bases ('N') with a non-zero quality scores\n"+
			      "\tThis program takes a BAM file as input and produces\n"+
			      "\tanother where unresolved bases have\n"+
    ...
    string pID          = "decrQuals";   
    string pName        = "decrQualNs";   
    string pCommandLine = "";
	...
	for(unsigned int i=0;i<al.QueryBases.length();i++){ //check all bases
	    if(toupper(al.QueryBases[i]) == 'N'){
		al.Qualities[i]=char(offset+baseQualForDeam);
	    }
	}
