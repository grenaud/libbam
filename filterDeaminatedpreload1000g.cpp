...
int main (int argc, char *argv[]) {
    ...
    string usage=string(""+string(argv[0])+"  [in BAM file]  [chr name] [deam out BAM] [not deam out BAM]"+
			"\nThis program divides aligned single end reads into potentially deaminated\n"+
			"\nreads and the puts the rest into another bam file if the deaminated positions are not called as the human reference.\n"+
			//"\nThis is like filterDeaminated but it loads the VCF before then labels the reads instead of doing it on the fly\n"+
	...
	thousandGenomesHasA  = new bool[ 250000000 ];
	thousandGenomesHasT  = new bool[ 250000000 ];
    }catch(bad_alloc& exc){
    ...
    string bamfiletopen = string( argv[ argc-4 ] );
	...
	if(al.IsReverseStrand()){
	    ...
	    readBase=toupper(         al.QueryBases[i]);
	    //refeBase  == 'C' 
	    if( readBase  == 'T' && refeBase  == 'C' &&  int(al.Qualities[i]-offset) >= minBaseQuality){ 
